...
\begin{figure}[H]
    ...
    \caption{Sulla sinistra AST, sulla destra albero di derivazione della parola \(digit+digit\)}
    \label{fig:ast-first-example}
\end{figure}
Si noti come l'albero della sintassi astratta a sinistra sia molto più compatto.

Ora affrontiamo invece la questione di come un AST venga salvato in memoria: di fatto questa struttura va a creare un grafo in cui ogni nodo rappresenta un valore intermedio del calcolo rappresentato dalla parola che stiamo analizzando. Il grafo (sì, lo chiamiamo tree ma può essere un grafo) ottenuto dalla semplificazione dell'albero di derivazione viene quindi salvato nodo per nodo nel seguente modo:
\begin{itemize}
    ...
    \item i nodi intermedi del grafo vengono salvati come struttura topologica dell'albero (da chiarire?) e rappresentano tutte le operazioni compiute tra variabili o valori diretti (quindi tutte le operazioni tra foglie).
\end{itemize}

\subsection{Creazione di un AST}
L'AST si può creare naturalmente dopo aver ottenuto l'albero di derivazione tramite delle operazoni di compattazione, tuttavia il nostro obiettive è quello di calcolare l'AST direttamente durante la fase di parsing, ovvero direttamente durante la fase di analisi sintattica.
Per fare questo dobbiamo immaginare di avere a disposizione due funzioni:
\begin{itemize}
    \item \texttt{newLeaf(label, val)} va a salvare un nodo foglia con due campi: l'etichetta ed il valore;
    \item \texttt{newNode(label, c1, ..., ck)} crea un nodo con \(k\) figli, l'etichetta è \texttt{label} mentre \texttt{c1, ..., ck} sono le referenze ai nodi figli;
\end{itemize}
...
Data la grammatica LALR per operazioni algebriche di somma e sottrazione 
\begin{align}
...
\begin{align*}
    E &\to E_1+T & & \{E.node = newNode('+', E_1.node, T.node)\} \\
    ecc.
\end{align*}
Analizziamo velocemente le regole più significative.
\(newLeaf(num, num.lexval)\) va a prendere il valore di num che ci è dato dall'analisi lessicale; in modo simile \(newLeaf(id, id.entry)\) ava acreare un nodo foglia in cui abiamo come etichetta l'identificatore mentre il valore del nodo si troverà all'interno della tabella dei simboli compilata in fase di analisi lessicale.
...
Ora che abbiamo le regole da applicare per cocstruire un alber di sintassi astratta, andiamo a costruire un AST per la parola \(id-num+id\) rispetto alla grammatica appena vista (\ref{gr:LALR-plus-minus-grammar}), immaginando che la parola \(id-num+id\) sia riavata dall'analisi lessicale di \(a-4+c\).

\noindent Innanzitutto dobbiamo arrivare alla fase del parsing, quindi ci dobbiamo procurare la tabella di parsing.

\noindent O almeno questo ci saremmo aspettati da un esercizio normale, ma invece no! La nostra guida spirituale a questo punto ci fa notare che abbiamo imparato a fare le tabelle di parsing per imparare a non farle più: dato che questa grammatica è SLR oltre che LALR e ci interessano solo le caselle in cui andremmo a piazzare riduzioni all'interno della tabella, possiamo calcolare anche solo una parte della tabella di parsing SLR.  
Quello a cui siamo interessati è inserire le riduzioni nelle caselle dove compaiono i simboli che stanno nel follow del driver delle produzioni e poi sfruttare effettivamente solo quelle caselle della tabella.

Questo metodo Quaglieo è ancora al vaglio degli studiosi, tuttavia sembra plausibile che la spiegazione sia la seguente: dato che le regole per la costruzione dell'AST riguardano solo i punti in cui si applicano le riduzioni a noi interessa solo capire quali riduzioni sono applicate ed in quale ordine.

Di fatto tutto il parsing in realtà è effettuato proprio per calcolare quali riduzioni sono applicate ed in quale ordine, dei passaggi intermedi nei vari nodi senza riduzioni non ce ne può proprio fregar di meno; quindi possiamo applicare la strategia dell'ignorare tutte le caselle della tabella di parsing che non contengono mosse di riduzione.

Comunque, tornando a noi e saltando qualche passaggio (che forse verrà inserito in seguito, dipende da quanto verranno pagati ed onorati gli scrittori della dispensa), la sequenza di riduzioni che si ottiene dal parsing della parola \(id-num+id\) è la seguente:
\begin{align*}
    T &\to id \\
    E &\to T \\
    T &\to num \\
    ...
    E &\to E+T
\end{align*}
Si invita il lettore a fermarsi un attimo e rileggere queste riduzioni per convincersi che siano esattamente le reiduzioni che si ottengono dal parsing bottom-up della parola data; si faccia attenzione all'ordine in cui si incontrano le riduzioni, questo è proprio l'ordine che ci si aspetta dalla procedura bottom-up, ovvero l'ordine inverso della procedura che dallo start symbol ci porta alla parola desiderata.
...
Il goal che ci siamo fissati all'inizio di questo esercizio era di costruire \emph{on the fly} l'albero di sintassi astratta duante la fase di parsing per la parola \(id-num+id\) derivata grammatica LALR per le espressioni aritmetiche (\ref{gr:LALR-plus-minus-grammar}).

\noindent Abbiamo utilizzato uno delle funzioni di attribuzione semantica che prevedono di utilizzare i seguenti attributi sintetizzati per le riduzioni con non-terminali nel body:
\begin{itemize}
    \item[E-node] per le riduzioni con driver E;
    \item[T-node] per le riduzioni con driver T.
\end{itemize}
...
\begin{itemize}
    \item[Id.entry] per le variabili;
    \item[Num.lexval] per i valori numerici.
\end{itemize}

Abbiamo visto quindi che si può ricavare l'AST mentre si esegue shift-reduce.
L'albero di sintassi astratta in questo caso è stato costruito su una grammatica LALR con un SDD con tutti attributi sintetizzati, è questo che ci ha permesso di costruire l'albero di sintassi astratta mentre costruivamo l'albero di parsing.

\section{Vai Alghisius, parti pure da qui}
