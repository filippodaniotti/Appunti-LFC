...
Arriviamo finalmente a parlare degli alberi di sintassi astratta (Abstract Syntax trees) che altro non sono che una rappresentazione compatta degli alberi di derivazione che spesso viene utilizzata come rappresentazione intermedia.
...
Affrontiamo la questione di come un AST venga salvato in memoria: di fatto questa struttura va a creare un albero in cui ogni nodo rappresenta un passaggio intermedio del processo descritto dalla parola che stiamo analizzando. L'AST ottenuto dalla semplificazione dell'albero di derivazione viene quindi salvato nodo per nodo nel seguente modo:
\begin{itemize}
    \item le foglie vengono salvate come link a record nella tabella dei simboli, di fatto creeremo una foglia per ogni identificatore ed ogni valore diretto (es.: valore numerico) presente nella parola che stiamo parsando;
    \item i nodi intermedi dell'albero costituiscono la struttura topologica dell'albero e rappresentano tutte le operazioni compiute tra variabili o valori diretti (quindi tutte le operazioni tra foglie).
\end{itemize}
...
\begin{itemize}
    ...
    \item \texttt{newNode(label, c1, ..., ck)} che crea un nodo con \(k\) figli, l'etichetta è \texttt{label} mentre \texttt{c1, ..., ck} sono i riferimenti ai nodi figli;
\end{itemize}
Applicando queste due funzioni all'esempio precedente (Fig.\ref{fig:ast-first-example}) avremo il nodo \(+\) che avrà come etichetta \(+\) e come figli i due nodi \(digit\).
I nodi \(digit\) saranno nodi foglia con identificatore e valore relativo (il valore è quello che ci viene ritornato dall'analisi lessicale, quindi lo possiamo ricavare con \(digit.lexval\)).\\

\noindent Per creare l'albero di sintssi astratta durante la fase di parsing dobbiamo definire delle regole semantiche (dette anche funzioni di attribuzione) che ci indichino quando e come istanziare un nodo per l'AST durante il parsing.
...
\(newLeaf(num, num.lexval)\) va a prendere il valore di num che ci è dato dall'analisi lessicale; in modo simile \(newLeaf(id, id.entry)\) va acreare un nodo foglia in cui abiamo come etichetta l'identificatore mentre il valore del nodo si troverà all'interno della tabella dei simboli compilata in fase di analisi lessicale.

Per le altre regole il significato, abbastanza intuitivo, è quello della creazine di un nodo intermedio che mantiene come informazioni l'etichetta dell'operazione da compiere e i riferimenti ai nodi degli operandi necessari.

\subsubsection{Esempio di creazione di un AST}
Ora che abbiamo le regole da applicare per cocstruire un albero di sintassi astratta, andiamo a costruirne uno per la parola \(id-num+id\) derivata dalla grammatica appena vista (\ref{gr:LALR-plus-minus-grammar}), immaginando che la parola \(id-num+id\) sia arrivata dall'analisi lessicale di \(a-4+c\).

\noindent Innanzitutto dobbiamo arrivare alla fase del parsing, quindi ci dobbiamo procurare la tabella di parsing.

\noindent O almeno questo ci saremmo aspettati da un esercizio normale, ma invece no! La nostra guida spirituale a questo punto ci fa notare che abbiamo imparato a fare le tabelle di parsing per imparare a non farle più: dato che questa grammatica è SLR oltre che LALR e ci interessano solo le caselle in cui andremmo a piazzare riduzioni all'interno della tabella, ci basta calcolare una parte della tabella di parsing SLR.

Quello a cui siamo interessati è solo capire per quali simboli si utilizzano quali riduzioni, quindi siamo interessati solo alle caselle della tabella in cui si vanno ad inserire le mosse di reduce.

Questo metodo Quaglieo è ancora al vaglio degli studiosi, tuttavia sembra plausibile che la spiegazione sia la seguente: dato che le regole per la costruzione dell'AST riguardano solo le riduzioni applicate per parsare la parola, a noi interessa solo capire quali riduzioni sono applicate ed in quale ordine.

Di fatto tutto il parsing in realtà è effettuato proprio per calcolare quali riduzioni sono applicate ed in quale ordine: dei passaggi intermedi nei vari stati senza riduzioni non ce ne può proprio fregar di meno; quindi possiamo applicare la strategia dell'ignorare tutte le caselle della tabella di parsing che non contengono mosse di riduzione.

... inserire qui eventuale tabella di parsing anche se non ci serve ... inserire qui parsing della parola anche se Lady Quail non l'ha fatto (cioè, l'ha fatto ma a mente)...

Comunque, tornando a noi e saltando qualche passaggio (che forse verrà inserito in seguito, dipende da quanto verranno pagati e riveriti gli scrittori della dispensa), la sequenza di riduzioni che si ottiene dal parsing della parola \(id-num+id\) è la seguente:
\begin{align*}
    T &\to id \\
    E &\to T \\
    T &\to num \\
    ...
    E &\to E+T
\end{align*}
Si invita il lettore a fermarsi un attimo e rileggere queste riduzioni per convincersi che siano esattamente le riduzioni che si ottengono dal parsing bottom-up della parola data; si faccia attenzione all'ordine in cui si incontrano le riduzioni, questo è proprio l'ordine che ci si aspetta dalla procedura bottom-up, ovvero l'ordine inverso della procedura che dallo start symbol ci porta alla parola desiderata.

Vediamo ora come viene creato in memoria l'Abstract Syntax Tree mentre stiamo svolgendo il parsing della stringa.
Ogni volta che incontriamo un riduzione andiamo ad aggiungere un nodo all'AST che stiamo creando. Quale forma avrà questo nodo ci è dato dalle funzioni di attribuzione dei nodi dell'AST; queste funzioni di attribuzione vengono chiamate qualche volta azioni semantiche.
...
\begin{enumerate}
    ...
    \begin{figure}[H]
        ...
        \caption{Creaione del nodo foglia per l'identificatore che identifica \(a\)}
    \end{figure}
    ...
    \end{figure} 
    \item In seguito ci scontriamo di nuovo in una riduzione \(T \to id\), che ha come funzione di attribuzione \(\{T.node = newLeaf(id, id.entry)\}\), sappiamo già come funziona dal primo putno di questa lista. La rappresentazione grafica del nostro AST a questo stadio è la seguente:
    \begin{figure}[H]
        \centering
        \includegraphics[width=.6\textwidth]{ex-ast-creation-5.png}
        \caption{Creazione di un nodo foglia per \(id\) associato a \(c\)}
    \end{figure} 
    \item Infine applichiamo la riduzine \(E \to E_1 + T\), che prevede l'azione semantica \(\{E.node = newNode ('+', E_1.node, T.node)\}\) e va a completare il nostro AST che possiamo finalmente osservare in tutta la sua bellezza:
    \begin{figure}[H]
        \centering
...
\paragraph{Riassumendo} Il goal che ci siamo fissati all'inizio di questo esercizio era di costruire \emph{on the fly} l'albero di sintassi astratta duante la fase di parsing per la parola \(id-num+id\) derivata grammatica LALR per le espressioni aritmetiche (\ref{gr:LALR-plus-minus-grammar}).

\noindent Abbiamo utilizzato delle funzioni di attribuzione semantica che prevedono di inserire i seguenti attributi sintetizzati per le riduzioni con non-terminali nel body:
\begin{itemize}
...
\subsection{Esempio di creazione di un AST ma con attributi ereditati}
...
Per visualizzare la situazione possiamo andare a ri-vedere l'SDD che abbiamo costruito per le operazioni \(+\) e \(*\) drante le prime lezioni dell'analisi semantica (Fig.\ref{fig:second-ex-parse-tree-part}).
...
Il problema in questo caso specifico è che non possiamo sintetizzare tutte le informazioni che ci servono perchè, uno degli elementi necessari per computare il valore dell'espressione e per la formazione dell'abstract syntax tree, non è presente nello stesso sottoalbero nel quale troviamo l'operatore aritmetico e l'altro operando coinvolto nell'operazione. 

Utilizzando gli attributi ereditati non si riesce ad eseguire in maniera immediata tutte le operazioni durante il parsing della grammatica: un'opzione, come già citato più volte, potrebbe essere quella di agire sulla grammatica in modo che, durante un'esecuzione del parsing bottom-up, si possa ottenere lo stesso risultato di una grammatica S-Attribuita. Senza soffermarci troppo sui dettagli, tale operazione passerebbe per l'aggiunta di marker e \(\epsilon\)-transizioni in modo che gli attributi vengano sintetizzati e non ereditati.

In questo caso per poter trasportare le informazioni da un ramo dell'albero ad un altro è necessario utilizzare degli attributi ereditati: in modo simile a quanto fatto per computare il valore delle espressioni aritmetiche, in questo caso assegniamo dei riferimenti a dei nodi. 
...
Dal Parse Tree possiamo osservare in modo più esplicito che il calcolo del sottoalbero destro necessita del sottoalbero sinistro e quindi avremo bisogno degli attributi ereditati (quindi non potremo eseguire il parsing e la costruzione dell'Abstract Syntax Tree contemporaneamente).
...
A questo punto dobbiamo definire le \textbf{regole} associate alle produzioni: tale operazione come sempre non è facile da eseguire ma vedremo di analizzarla passo per passo. 

Cominciamo inserendo le regole delle produzioni finali della grammatica proposta: queste definiscono le foglie del nostro Parse Tree e dovranno occuparsi di generare gli elementi finali dell'Abstract Syntax Tree.
...
\begin{itemize}
    ...
    \item mentre l'altra con etichetta num e valore num.lexval (i.e. il valore ottenuto dall'analisi lessicale)
\end{itemize}
...
Volendo chiarire subito alcuni possibili dubbi, sottolineo il fatto che questa non sarà l'unica regola che sarà associata a tale produzione (si è voluto optare per un aggiunta graduale) e che la produzione a cui è stata collegata la regola è quella dove i due nodi fratelli compaiono nel body.
...
A questo punto abbiamo bisogno dell'\emph{Evaluation Order}, ottenuto facendo l'ordinamento topologico del Dependency Graph; si tenga a mente che un qualsiasi ordinamento topologico del grafo può andare bene: è possibile eseguire una visita in profondità e, prima di risalire nella ricorsione, inserire i nodi in uno stack; rimuovere per primi i nodi che non hanno archi entranti; etc... Ricordiamo anche che non è detto che si riesca a trovare un ordinamento topologico: in questo caso non è può valutare il grafo ottenuto.
...
\begin{enumerate}
    ...
    \item Generiamo la foglia \emph{Leaf(id, 1)} (i.e. una foglia con etichetta id e un riferimento alla Symbol Table) e manteniamo un riferimento ad essa: tale operazione viene eseguita dalla regola \(\{T.node = newLeaf(id, id.entry)\}\) per  \(T \to id\)
    \item Recuperiamo il valore di \(num\) ottenuto dall'analisi lessicale mediante \(num.lexval\) (i.e. 4)
    \item Generiamo la foglia \emph{Leaf(num, 3)} e manteniamo un riferimento ad essa: tale operazione viene eseguita dalla regola \(\{T.node = newLeaf(num, num.lexval)\}\) per \(T \to num\)
    \item Creiamo un riferimento a \emph{Leaf(id, 1)} utilizzando la regola \(\{E'.i = T.node\}\) per  \(E \to TE'\). In questo passaggio dobbiamo recuperare \(T.node\) (che contiene un riferimento alle informazioni del sottoalbero sinistro) e assegnarlo a \(E'.node\), in modo che i passi successivi possano usufruirne
    \item Generiamo il nodo \emph{Node(-, 5, 4)} e manteniamo un riferimento ad esso: tale operazione viene eseguita dalla regola \(\{E_{1}'.i = newNode(-, E'.i, T.node)\}\) per \(E' \to -TE_{1}'\); anche in questo caso abbiamo bisogno di passare il riferimento ai passi successivi
    \item Creiamo un riferimento a \emph{Node(-, 5, 4)} utilizzando la regola \(\{E'.node = E'.i\}\) per \(E' \to \epsilon\)
    \item Creiamo un riferimento a \emph{Node(-, 5, 4)} utilizzando la regola \(\{E'.node = E_{1}'.node\}\) per \(E' \to -TE_{1}'\)
    \item Creiamo un riferimento a \emph{Node(-, 5, 4)} utilizzando la regola \(\{E.node = E'.node\}\) per \(E \to TE'\)
\end{enumerate}
...
\subsubsection{Es aggiornamento tipo degli identificatori}
...
L'obbiettivo è quello di associare alle produzioni descritte delle regole per cui si possa aggiungere il tipo nella entry della Symbol Table degli identificatori dichiarati. Per poter aggionrare la tabella è necessario utilizzare la funzione addtype(table\_entry), type\_instance).
