...
\end{itemize}
Il lettore accorto si sarà senz'altro reso conto che non abbiamo definito degli attributi per i caratteri terminali, questo perché gli attributi dei terminali sono sempre noti: sono dati dall'analisi lessicale e di conseguenza non serve alcuna regola per calcolarli.
...
\subsection{Esempio}
Prendiamo come esempio la nostra grammatica per la valutazione delle espressioni aritmetiche per valutare l'espressione rappresentata dall'albero di derivazione in Fig.\ref{fig:ex_albero_derivazione_somma_1}.
\begin{align*}
    V &\to E \\
    E &\to E + T \mid T\\
    T &\to T * F \mid F\\
...
\begin{figure}[H]
    \centering
    \subimport{assets/figures}{ex_albero_derivazione_somma_1.tex}
    \caption{Albero di derivazione per una somma}
    \label{fig:ex_albero_derivazione_somma_1}
\end{figure}
...
\begin{itemize}
    ...
    \item[Reg.\ref{reg:regola-3}] se ho una produzione \(E \to T\) il valore di \(E\) è uguale al valore di \(T.val\);
...
Si può capire al volo cosa intendiamo guardando il parse tree annotato corrispondente al parse tree precedente (Fig.\ref{fig:ex_albero_derivazione_somma_2}) che è raffigurato qui sotto.

% Using Figure instead of Image
\begin{figure}[H]
    \centering
    \subimport{assets/figures/}{ex_albero_derivazione_somma_2.tex}
    \caption{Albero di derivazione per una somma in \ref{fig:ex_albero_derivazione_somma_1}}
    \label{fig:ex_albero_derivazione_somma_2}
\end{figure}

% \begin{figure}[H]
%     \centering
%     \includegraphics[width=.8\textwidth]{first-ex_noted-parse-tree.jpg}
%     \caption{Parse tree annotato per la somma in \ref{fig:first-ex_SDD}}
...
\begin{enumerate}
    ...
    \begin{figure}[H]
        \centering
        \subimport{assets/figures/}{res_albero_derivazione_somma_1.tex}
    ...
    \begin{figure}[H]
        \centering
        \subimport{assets/figures/}{res_albero_derivazione_somma_2.tex}
    ...
    \begin{figure}[H]
        \centering
        \subimport{assets/figures/}{res_albero_derivazione_somma_3.tex}
    ...
    \begin{figure}[H]
        \centering
        \subimport{assets/figures/}{res_albero_derivazione_somma_4.tex}
    ...
    \begin{figure}[H]
        \centering
        \subimport{assets/figures/}{res_albero_derivazione_somma_5.tex}
...
% Using Figure instead of Images

\begin{figure}[H]
    \centering
    \subimport{assets/figures/}{dependency_graph_ex_albero_derivazione_somma.tex}
    \caption{Albero di parsing annotato e relativo grafo delle dipendenze}
    \label{fig:dependency_graph_ex_albero_derivazione_somma}
\end{figure}
...
% Using PseudoCode instead of Image
\begin{figure}[H]
    \centering
    \subimport{assets/pseudocode/}{postorder-visit.tex}
...
\begin{figure}[H]
    \centering
    \subimport{assets/figures/}{dependence_conflict.tex}
    \caption{Conflitto di dipendenza}
    \label{fig:dependence_conflict}
\end{figure}
...
\begin{figure}[H]
    \centering
    \subimport{assets/figures/}{ex_parse_tree.tex}
    \caption{Pezzo del parse tree di \(3*5\)}
    \label{fig:ex_parse_tree}
\end{figure}
...
\noindent Questo è il caso in cui possiamo aiutarci con l'utilizzo di attributi ereditati, presentiamo in Fig.\ref{fig:dependency_graph_ex_parse_tree_1} come si possono utilizzare gli attributi ereditati; in blu abbiamo ancora una volta gli archi del grafo delle dipendenze e le etichette \(.i\) e \(.s\) rappresentano rispetivamente attributi ereditati e sintetizzati.
% Using Figure instead of Image
\begin{figure}[H]
    \centering
    \subimport{assets/figures/}{dependency_graph_ex_parse_tree_1.tex}
    \caption{Pezzo del grafo delle dipendenze di \(3*5\)}
    \label{fig:dependency_graph_ex_parse_tree_1}
\end{figure}
...
\begin{figure}[H]
    \centering
    \subimport{assets/figures/}{dependency_graph_ex_parse_tree_2.tex}
    \label{fig:dependency_graph_ex_parse_tree_2}
\end{figure}
...
Se andiamo invece ad osservare come viene generato il valore di \(T'\) vediamo dal pezzo di parse tree annotato in Fig.\ref{fig:dependency_graph_ex_parse_tree_1} che in pirmis ereditiamo un elemento dal fratello di sinistra, poi sappiamo che lo moltiplichiamo per il valore che ci è dato da \(F.s\) per ottenere il valore di \(T'_1\), che poi passeremo a \(T'\).
...
\begin{figure}[H]
    \centering
    \subimport{assets/figures/}{dependency_graph_ex_parse_tree_3.tex}
    \label{fig:dependency_graph_ex_parse_tree_3}
\end{figure}
...
\begin{figure}[H]
    \centering
    \subimport{assets/figures/}{lalr_automata.tex}
\end{figure}
...
\begin{figure}[H]
    \centering
    \subimport{assets/figures/}{ex_ast_1.tex}
...
\end{itemize}
Applicando queste due funzioni all'esempio precedente (Fig.\ref{fig:ex_ast_1}) avremo l'operazione \(+\) rappresentata da un nodo che avrà come etichetta \(+\) e come figli i due nodi \(digit\).
...
%     \begin{figure}[H]
%         \centering
%         \subimport{assets/figures/}{ex_ast_creation_1.tex}
...
%     \begin{figure}[H]
%         \centering
%         \subimport{assets/figures/}{ex_ast_creation_2.tex}
...
%     \begin{figure}[H]
%         \centering
%         \subimport{assets/figures/}{ex_ast_creation_3.tex}
...
%     \begin{figure}[H]
%         \centering
%         \subimport{assets/figures/}{ex_ast_creation_4.tex}
...
%     \begin{figure}[H]
%         \centering
%         \subimport{assets/figures/}{ex_ast_creation_5.tex}
...
%     \begin{figure}[H]
%         \centering
%         \subimport{assets/figures/}{ex_ast_creation_6.tex}
...
\begin{enumerate}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=.6\textwidth]{ex-ast-creation-1.png} % TODO: trasformare in figure
        \caption{Creaizone del nodo foglia per l'identificatore che referenzia la variabile \(a\)}
    \end{figure}
    \item La seconda riduzione che incontriamo, \(E \to T\), ci porta ad un semplice passaggio di rinominazione di un nodo dell'AST, in quanto la sua azione semantica è \(\{E.node = T.node\}\).
    \begin{figure}[H]
        \centering
        \includegraphics[width=.6\textwidth]{ex-ast-creation-2.png} % TODO: trasformare in figure
        \caption{Rinominazione del \(T.node\) in \(E.node\)}
    \end{figure} 
    \item Il terzo passo ci porta ad usare la riduzione \(T \to num\), con annessa funzione di attribuzione \(\{T.node = newLeaf(num, num.lexval)\}\), che crea una nuova foglia con valore numerico associato.
    \begin{figure}[H]
        \centering
        \includegraphics[width=.6\textwidth]{ex-ast-creation-3.png} % TODO: trasformare in figure
        \caption{Creazione di una nuova foglia con valore numerico associato}
    \end{figure} 
    \item La prossima riduzione che incontriamo è \(E \to E_1 - T\), con azione semantica \(\{E.node = newNode('-', E_1.node, T.node)\}\), che viene rappresentata graficamente creando un nuovo nodo intermedio per l'AST, la cui forma può essere osservata nella figura sottostante.
    \begin{figure}[H]
        \centering
        \includegraphics[width=.6\textwidth]{ex-ast-creation-4.png} % TODO: trasformare in figure
        \caption{Inserimento del nodo intermedio che rappresenta la sottrazione}
    \end{figure} 
    \item In seguito ci scontriamo di nuovo in una riduzione \(T \to id\), che ha come funzione di attribuzione \(\{T.node = newLeaf(id, id.entry)\}\), sappiamo già come funziona dal primo punto di questa lista. La rappresentazione grafica del nostro AST a questo stadio è la seguente:
    \begin{figure}[H]
        \centering
        \includegraphics[width=.6\textwidth]{ex-ast-creation-5.png} % TODO: trasformare in figure
        \caption{Creazione di un nodo foglia per \(id\) associato a \(c\)}
    \end{figure} 
    \item Infine applichiamo la riduzione \(E \to E_1 + T\), che prevede l'azione semantica \(\{E.node = newNode ('+', E_1.node, T.node)\}\) e va a completare il nostro AST che possiamo finalmente osservare in tutta la sua bellezza:
    \begin{figure}[H]
        \centering
        \includegraphics[width=.6\textwidth]{ex-ast-creation-6.png} % TODO: trasformare in figure
        \caption{Inserimento del nodo intermedio per la rappresentazione della somma}
    \end{figure} 
\end{enumerate}
...
Per visualizzare la situazione possiamo andare a rivedere l'SDD che abbiamo costruito per le operazioni \(+\) e \(*\) nella prima parte riguardo l'analisi semantica (Fig.\ref{fig:ex_parse_tree}, ma si può osservare tranquillamente la figura  qui sotto per rendersi conto).
...
\begin{figure}[H]
    \centering
    \subimport{assets/figures/}{ex_parse_tree_attributi_ereditati.tex}
    \caption{Parse tree attribuiti ereditati}
    \label{fig:parse_tree_ae}
\end{figure}
...
\begin{figure}[H]
    \centering
    \subimport{assets/figures/}{dependency_graph_ex_parse_tree_ae.tex}
    \caption{Dependency graph attributi ereditati}
    \label{fig:dependency_graph_ae}
\end{figure}
...
\begin{figure}[H]
    \centering
    \subimport{assets/figures/}{dependency_graph_ex_parse_tree_ae_evaluation_order.tex}
    \caption{Dependency Graph attributi ereditati con Evaluation Order}
    \label{fig:dependency_graph_ae_evaluation_order}
\end{figure}
...
\begin{figure}[H]
	\centering
    \includegraphics[width=.8\textwidth]{abstract-syntax-tree-ae.jpg} % TODO: trasformare in figure
...
\begin{figure}[H]
	\centering
    \subimport{assets/figures/}{parse_tree_int_float.tex}
    \caption{Parse tree aggiornamento tipo identificatori}
    \label{fig:parse_tree_int_float}
\end{figure}
