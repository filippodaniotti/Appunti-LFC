...
Sia \(D = (S,A,move_{d},s_{0},F)\) un DFA con funzione di transizione \emph{totale}. Allora due stati \(s,t \in S\)) si dicono equivalenti se e solo se la seguente condizione è verificata:


\(move_{d}^{*}(s,x) \in F\) se e solo se \(move_{d}^{*}(t,x) \in F  \forall x \in A^{*}\) 
dove la funzione multi-passo \(move_{d}^{*}\) è definita per induzione nel seguente modo:


\(move_{d}^{*}(s,\epsilon)=s\) 

\(move_{d}^{*}(s,wa)= move_{d}(move_{d}^{*}(s,w),a)\)
...
Iniziamo con 2 blocchi \(B_{1}=F\) e \(B_{2}=S\setminus F\). Questa scelta fa sì che i due blocchi non abbiano stati equivalenti in quanto con \(s\in B_{1}\) e \(t\in B_{2}\) vale che \(move_{d}^{*}(s,\epsilon)\in F\) e \(move_{d}^{*}(t,\epsilon)\notin F\)\\
...
Il blocco \(B_{i}\) può essere spezzato in due se per qualche \(s,t \in B_{i}\) vale \(move_{d}(s,a)\in B_{j}\) e \(move_{d}(t,a)\notin B_{j}\)
...
\begin{figure}[H]
	\centering
	\includegraphics[width=.8\textwidth,keepaspectratio]{algoritmo_partiton_refinement.png}
...
possiamo fare uno \emph{split} di \(B_{2}\) in quanto \(move_{d}(A,a) \in B_{2} \) e \(move_{d}(B,a) \notin B_{2} \)
da cui otteniamo i nuovi blocchi \(B_{21}=\{A,Sink\}\) e \(B_{22}=\{B,C\}\).
Possiamo fare un ulteriore \emph{split} di \(B_{22}\) in quanto \(move_{d}(B,b) \in B_{22} \) e \(move_{d}(C,b) \notin B_{22} \)
da cui otteniamo i blocchi \(B_{221}=\{B\}\) e \(B_{222}=\{C\}\).
Infine notiamo che è possibile uno \emph{split} di \(B_{21}\) in quanto \(move_{d}(A,a) \in B_{221} \) e \(move_{d}(Sink,a) \notin B_{221} \). 
