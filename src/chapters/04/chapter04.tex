...
\begin{itemize}
    \item l'espressione regolare $(a^*(bb)^*)^* b(a^*(bb)^*)^*$ non può denotare il linguaggio descritto poichè $ababab \notin \mathcal{L}_1$
    ...
    \item l'espressione regolare $(a^*(b)a^*)(a^*(b)a^*(b)a^*)^*$ sembrerebbe essere corretta in quanto sicuramente accetterò parole composte da almeno una $b$ e un numero arbitrario di $a$ prima e dopo; nel caso in cui volessi aumentare il numero di $b$ sono obbligato a farlo a coppie con la possibilità di interporre prima, in mezzo o alla fine un numero arbitrario di $a$. 
\end{itemize}

In questo caso, visto che siamo stati in grado di trovare un DFA con un numero finito di stati, possiamo affermare che $\mathcal{L}_1$ è un linguaggio regolare.

Come possiamo però essere certi del fatto che ad esempio l'espressione regolare $(a^*(b)a^*)(a^*(b)a^*(b)a^*)^*$ denota il linguaggio $\mathcal{L}_1$ e che quindi è anch'essa soluzione del quesito? Il modo corretto di procedere è il seguente
...
\begin{enumerate}
    ...
    \item Si applica l'algoritmo per la \emph{DFA minimization} (aggiungendo dunque lo stato pozzo se necessario) per ridurre il numero di stati
    \item Si osserva se il DFA ridotto è in realtà un isomorfismo del DFA proposto come soluzione
\end{enumerate}

Notare che in questo caso una soluzione possibile (proposta dalla prof.ssa) è $(b a^*b \mid a)^* ba^*$
...
Visto che 0 è considerato pari in questo caso lo stato iniziale coincide con quello finale. Le transizioni devono essere opportunamente costruite per cui se voglio aggiungere una $a$ alla mia parola devo essere costretto a seguire una $a$-transizione in uno stato non finale per poi tornare in quello finale con un'ulteriore $a$-transizione in modo da assicurare che le condizioni siano rispettate. Ovviamente deve esservi sempre la possibilità di inserire un numero arbitrario di $b$ tra le $a$ per cui in entrambi gli stati si ha una $b$-transizione sullo stesso stato. Analogamente all'esercizio precedente si ha che $\mathcal{L}_2$ è un linguaggio regolare.

In questo caso l'espressione regolare proposta è stata $(b^*ab^*ab^*)^*$ 
...
Una soluzione tanto banale quanto funzionale consiste nel utilizzare la Thompson's Construction per poter concatenare gli automi descritti negli esercizi precedenti con l'operazione di \emph{Alternation}. A livello formale questo si traduce come $\mathcal{L}(r_1 \mid r_2) = \mathcal{L}(r_1) \cup \mathcal{L}(r_2)$. Essendo che $\mathcal{L}_1$ e $\mathcal{L}_2$ sono linguaggi regolari, l'unione dei due così descritta non può che essere anch'essa un linguaggio regolare.
...
\end{itemize}
Ovviamente il nostro stato iniziale non può essere altro che quello dove si hanno 0 occorrenze di $a$ e di $b$ (il primo descritto) mentre lo stato finale dovrà per forza essere il secondo dell'elenco.

Dunque quello che abbiamo appena immaginato di costruire è un'automa che possiede 4 possibili stati di cui soltanto uno deve essere considerato quello finale.

C'era un modo per arrivare direttamente alla soluzione senza dover fare un ragionamento di questo tipo ma pensando all'intersezione: immaginiamo di avere una parola $w$ a disposizione e di poter, contemporaneamente, muoverci su entrambi i cammini degli automi descritti come soluzioni di $\mathcal{L}_1$ e di $\mathcal{L}_2$. Continuiamo a spostarci eseguendo le transizioni coerentemente con la parola $w$ scelta fino a che non arriviamo al carattere terminatore; a questo punto sono dati due casi:
...
I linguaggi regolari sono chiusi wrt

\begin{itemize}
	\item unione, perchè se prendiamo due linguaggi regolari l'unione di questi linguaggi è ancora un linguaggio regolare. Come nel caso precedente essendo che i linguaggi sono regolari allora è possibile costruire un DFA sia per il primo che per il secondo, se a questo punto si prendono i due automi così costruiti e legano tramite una $\epsilon$-transizione, per l'operazione di \emph{Alternation} si ottiene nuovamente un linguaggio regolare
	\item concatenazione, perchè è possibile sfruttare l'operazione di \emph{Concatenation} della  Thompson's Construction ed ottenere ancora una volta un'automa che denota un linguaggio regolare 
	\item complementazione, perchè se $\mathcal{L}$ è un linguaggio regolare su un certo alfabeto $\mathcal{A}$, la complementazione è data dal linguaggio delle parole su $\mathcal{A} \setminus \mathcal{L}$. Se un linguaggio è regolare anche il suo complemento è regolare. 
	\item intersezione, perchè è derivabile dall'unione utilizzando \emph{De Morgan}: $\mathcal{L}_1 \cap \mathcal{L}_2 = \lnot (\lnot (L1 \cap L2)) = \lnot (\lnot \mathcal{L}_1 \cup \lnot \mathcal{L}_2)$.
\end{itemize}

In precedenza abbiamo visto l'automa per riconoscere $\mathcal{L}_2$, cioè che accettasse tutte quelle parole dove $a$ occorreva un numero pari di volte. Il complementare di tale automa corrisponde a quello per cui si hanno parole con un numero dispari di occorrenze. Utilizzando l'automa che denota $\mathcal{L}_2$, vi è la possibilità di applicare una particolare operazione per trasformarlo nel suo complementare? E' possibile fare quanto descritto semplicemente rendendo finali tutti quegli stati che prima non lo erano e non finali tutti quelli che prima lo erano.

Relativamente questo procedimento è però necessario fare una precisazione e cioè che tale metodo funziona solamente se il DFA presenta una funzione di transizione \textbf{totale}: intuitivamente è necessario riconoscere tutte quelle parole che non appartengono al linguaggio regolare in quanto queste apparterranno al linguaggio del DFA complementare; se per assurdo si avesse la mancanza di una $a$-transizione (non fondamentale nel primo DFA) non si avrebbe la possibilità di costruire il set completo delle parole.
