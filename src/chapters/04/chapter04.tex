...
Guardiamo il caso della parentesizzazione \(r = ( r_1 )\), il quale risulta estremamente banale, così tanto che l'interpretazione è lasciata al lettore.
...
\begin{itemize}
    ...
    \item se \(\textrm{move}_d\) è \emph{parziale}, allora per ogni stato c'è \textbf{almeno} una \(a\)-transizione \(\forall a \in \mathcal{A}\); in altre parole, per alcune coppie \((s,a) \in S \times A\) la funzione non è definita.
\end{itemize}
...
\begin{itemize}
    ...
    \item oppure vale che \(s_0 \in F \land w = \epsilon\); per l'appunto, quello in cui lo stato di partenza è uno stato finale è l'unico caso in cui un DFA possiede la parola vuota.
\end{itemize}
...
In generale si preferisce lavorare con DFA con funzione di transizione parziale, e ancora più in generale con automi che abbiamo il minor numero di stati (nodi e archi) possibile; tuttavia, l'algoritmo di minimizzazione (vedremo più avanti), che usiamo per "ridurre al minimo" il suo numero di archi e nodi, non funziona con DFA dotati di funzione di transizione parziale. Il motivo sta nel fatto che questa procedura lavora sulla funzione inversa di \(\textrm{move}_d\), che chiaramente non è definita se la funzione non è totale.
