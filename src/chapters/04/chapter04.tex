...
\end{figure}
Procediamo ora con l’algoritmo di partition refinement per minimizzare il precedetne DFA.

Il primo passo è quello di dividere in due blocchi: gli stati finali (\(B2\)) e gli stati non finali (\(B1\)).
In questo caso la partizione è la seguente:
\begin{equation*}
    B1 = \{A,B,C,D\} \textrm{ e } B2 = \{E,F,G,H\}
\end{equation*}

A questo punto procediamo con le raffinazioni successive. Se all’interno dello stesso blocco troviamo due stati che tramite una \(a\)-transizione portano a due blocchi distinti (con \(a\) terminale qualsiasi dell’alfabeto \(\mathcal{A}\)).

\noindent Nota: la scelta di quale blocco splittare influisce molto sull’efficienza dell’algoritmo, gli algoritmi più efficienti svolgono questa scelta in maniera mirata.

Decidiamo di splittare \(B1\) secondo \(move_d (A,a) \), dato che osservo che \(move_d (C,a)\)non ha lo stesso blocco target di \(move_d (A,a) \) (con una \(a\)-transizione da \(A\) raggiungo\(B1\), con una \(a\)-transizione da \(C\) raggiungo \(B2\) ).

In questo caso il risultato della partizione è il seguente:
\begin{equation*}
    \{A, B\};\; \{C, D\};\; \{E,F,G,H\}
\end{equation*}
Questa nuova partizione è ulteriormente splittabile?
Sì! Posso performare uno split su \(move_d(A, a)\), dato che attraverso una \(a\)-transizione da \(A\) mi muovo in \(\{A,B\}\) mentre da \(B\) mi muovo in \(\{C,D\}\) splitto il blocco \(\{A,B\}\).
Ottengo quindi questa nuova suddivisione:
\begin{equation*}
    \{A\};\; \{B\};\; \{C, D\};\; \{E,F,G,H\}
\end{equation*}
Ora procediamo ad oltranza con i raffinamenti successivi.

Posso splittare su \(move_d(G,b)\) e \(move_d(H,b)\); ottengo quindi:
\begin{equation*}
    \{A\};\; \{B\};\; \{C, D\};\; \{E,F,H\};\; \{G\}
\end{equation*}
Il prossimo è lo split su \(move_d(C, a)\) e \(move_d(D, a)\), che mi porta ad ottenere:
\begin{equation*}
    \{A\};\; \{B\};\; \{C\};\; \{D\};\; \{E,F,H\};\; \{G\}
\end{equation*}
Ed ora splitto su \(move_d(E, a)\) e \(move_d(H, a)\):
\begin{equation*}
    \{A\};\; \{B\};\; \{C\};\; \{D\};\; \{E,F\};\; \{H\};\; \{G\}
\end{equation*}
Infine posso splittare anche su \(move_d(E, a)\) ed \(move_d(F, a)\), quindi arrivo al raffinamento con la seguente forma:
\begin{equation*}
    \{A\};\; \{B\};\; \{C\};\; \{D\};\; \{E\};\; \{F\};\; \{H\};\; \{G\}
\end{equation*}
A questo punto ho raggiunto una partizione in cui tutti i blocchi contengono un solo stato.

In tal caso ho terminato lo svolgimento dell'algoritmo di partition refinement: questo significa che il DFA da cui sono partito (ottenuto da subset construction) era esattamente minimo.
...
\subsubsection{Lemma sulla dimensione di un DFA}
\begin{lemma}
...
\begin{proof}
    ...
    \begin{equation}
        \mathcal{L} = \mathcal{L}((a\mid b)^* a (a\mid b)^{n-1})  
        \label{linguaggio_lemma_limiti_dfa}
    \end{equation}
    Essendo questo un linguaggio regolare può essere rappresentato utilizzando un automa a stati finiti.
    Esiste un NFA con esattamente \(n+1\) stati che accetta il linguaggio \(\mathcal{L}\), questo automa è rappresentiato in Fig.\ref{nfa_lemma_limiti_dfa}.
    \begin{figure}[H]
        \centering
    ...
    Sappiamo esserci esattamente \(2^n\) parole distinte su \(\{a, b\}\) la ui lunghezza è \(n\).\\
    ...
    \end{itemize}
    Se non esistessero allora i nodi sarebbero proprio \(2^n\).
    Di conseguenza per qualche \(x_1\), \(x_2\) e \(x\) vale una delle due opzioni seguenti:
    \begin{enumerate}
        \item \(w_1 = x_1 a x\) e \(w_2 = x_2 b x\)
        \item \(w_1 = x_1 b x\) e \(w_2 = x_2 a x\)
    \end{enumerate}
    altrimenti le due parole \(w_1\) e \(w_2\) sarebbero uguali.\\
    Supponiamo in questo caso che sia valida la condizione 
    \begin{equation*}
        w_1 = x_1 a x \textrm{ e } w_2 = x_2 b x
    \end{equation*}
    Allora possiamo definire \(w_1'\) in questo modo
    \begin{equation*}
        w_1' = x_1 a b^{n-1} \in \mathcal{L}(\mathcal{D})
    \end{equation*}
    Di conseguenza lo stato riaggiunto da \(w_1'\) in \(\mathcal{D}\) è uno stato finale.\\
    Questa è una contraddizione: lo stato non può essere finale perchè può essere raggiunto anche tramite \(x_2 b b^{n-1}\) ma 
    \begin{equation*}
        x_2 b b^{n-1} \notin \mathcal{L}(\mathcal{D})
    \end{equation*}
    questo è l'assurdo che prova la validità del lemma.
\end{proof}
Tutto il lemma gioca sul fatto che un DFA ha bisogno esattamente di tutti quanti i possibili cammini che siano comporti da \(n-1\) alternanze di \(a\) e di \(b\) per esprimere lo stesso linguaggio espresso dall'NFA.

Questo lemma quindi ci offre una stima per il numero minimo di stati di un DFA che rappresenta un certo altro NFA.

\subsubsection{Pumpkin Lemma per linguaggi regolari}
\begin{lemma}
    Sia \(\mathcal{L}\) un linguaggio regolare, allora si ha che:
    \begin{itemize}
    ...
    \end{itemize}
\end{lemma}
Ovvero, come il lettore avrà ben notato, questo è il corrispettivo del pumping lemma per i linguaggi regolari.

Illustriamo ora la dimostrazione di questo lemma.
\begin{proof}
    Dato che \(\mathcal{L}\) è un linguaggio regolare, sappiamo che esiste un DFA \(\mathcal{D} = (S,A,move_d ,s_0, F)\) tale per cui \(\mathcal{L} = \mathcal{L}(\mathcal{D})\).
    ...
    Quindi se per una parola \(z\) vale \(|z|>p\), allora possiamo scomporre \(z\) in \(z = a_1 \dots a_p z'\) ed abbiamo la certezza che almeno uno stato, diciamo \(s^*\), sia attraversato più di una colta lungo il cammino \(a_1 \dots a_p\).

    Questo significa che esiste un ciclo in \(\mathcal{D}\) che parte da \(s^*\) e torna in \(s^*\) e questo ciclo si può indicare come \(a_{i+} \dots a_j\), con \(i<j\le p\).

    Definiamo ora \(u\), \(v\) e \(w\):
    \begin{itemize}
        \item \(u = a_1 \dots a_i\)
        \item \(v = a_{i+} \dots a_j\) \textrm{(quindi \(v\) rappresenta il ciclo)}
        \item \(w = 
        \begin{cases}
            z'                   & \textrm{if } j = p\\ 
            a_{j+1} \dots a_pz'  & \textrm{if } j < p
        \end{cases}\)
    \end{itemize}
...
\end{proof}
Abbiamo quindi dimostrato questa versione del pumping lemma per i linguaggi regolari, ora la domanda che sorge spontanea è: per cosa si utilizza questo lemma?

Come nel caso dei linguaggi liberi questo lemma viene utilizzato per dimostrare, tramite la contraddizione, che un linguaggio \emph{non} è regolare.

Ovvero si assume che un dato linguaggio \(\mathcal{L}\) sia regolare, se si riesce a dimostrare che per quel linguaggio la tesi non è soddisfatta (il negato della tesi è soddisfatto), allora si può constatare che \(\mathcal{L}\) in realtà non è un linguaggio regolare.

Ripetiamo, per amor di chiarezza, la tesi e ne scriviamo in seguito la negazione.
\begin{description}
    \item[Tesi] 
...
\begin{equation}
    \mathcal{L} = \{a^n b^n \mid n>0\} \textrm{ non è regolare}
...
Se \(\mathcal{L}\) fosse un linguaggio regolare dovrebbe essere possbile rappresentarlo con un NFA o un DFA; proviamo ad immaginare il DFA che riconosce questo linguaggio, come dovrebbe essere?

Per bilanciare il numero di \(b\) e di \(a\) dovrebbe in qualche modo obbligare l’inserimento di una \(b\) per ogni \(a\) inserita in precedenza, ma questo non si può fare a meno di inserire infiniti stati, il che è impossibile.

Procediamo quindi con la dimostrazione tramite pumping lemma. Supponiamo \(\mathcal{L}\) regolare, prendiamo \(p\) intero positivo arbitrario (il negato della tesi deve valere per ogni \(p\)).

Ora, costruiamo la parola \(z=a^p b^p\), abbiamo che \(|z|>p\).
Osserviamo che:
\begin{equation*}
    \forall \; u,v,w \textrm{ se } (z = uvw \textrm{ e })|uv| \le p \textrm{ e } |v| > 0
\end{equation*}
sappiamo che:
\begin{itemize}
...
\end{itemize}
Se noi ora definiamo \(j > 0\) tele per cui 
\begin{equation*}
    uv^2w = a^pa^jb^p 
\end{equation*}
abbiamo che \(uv^2w \notin \mathcal{L}\), il che contraddice il pumping lemma per i linguaggi regolari.

Abbiamo quindi dimostrato per contraddizione del pumping lemma per linguaggi regolari che l'affermazione in Eq.\ref{pl_regular_languages_ex_1} è verificata.
