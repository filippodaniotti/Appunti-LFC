...
Spesso e volentieri, non siamo interessati ad un generico \(D\), che genera un linguaggio \(\mathcal{L(D)}\), ma siamo invece interessati  ad avere il DFA minimo, \(D'\) con numero minimo di stati, tale che  \(\mathcal{L(D)} = \mathcal{L(D')}\) 


Prima di procedere, dobbiamo definire la nozione di \emph{State Equivalence}

Sia \(D = (S,A,\textrm{move}_{d},s_{0},F)\) un DFA con funzione di transizione \emph{totale}. Allora due stati \(s,t \in S\)) si dicono equivalenti se e solo se la seguente condizione è verificata:


\(\textrm{move}_{d}^{*}(s,x) \in F\) se e solo se \(\textrm{move}_{d}^{*}(t,x) \in F  \forall x \in A^{*}\) 
dove la funzione multi-passo \(\textrm{move}_{d}^{*}\) è definita per induzione nel seguente modo:


\(\textrm{move}_{d}^{*}(s,\epsilon)=s\) 

\(\textrm{move}_{d}^{*}(s,wa)= \textrm{move}_{d}(\textrm{move}_{d}^{*}(s,w),a)\)

L'idea generale dell'algoritmo di minimizzazione qui proposta è che alcuni stati siano ridondanti, e che dunque possano essere rimossi senza perdere informazione, ovvero senza modificare il linguaggio riconosciuto dal DFA.

Per eliminare gli stati in eccesso, utilizziamo la procedura di \emph{partition refinement}
\paragraph{Partition Refinement}
Partizioniamo gli stati in 2 blocchi ( insiemi di stati disgiunti)\\
Iniziamo con 2 blocchi \(B_{1}=F\) e \(B_{2}=S\setminus F\). Questa scelta fa sì che i due blocchi non abbiano stati equivalenti in quanto con \(s\in B_{1}\) e \(t\in B_{2}\) vale che \(\textrm{move}_{d}^{*}(s,\epsilon)\in F\) e \(\textrm{move}_{d}^{*}(t,\epsilon)\notin F\)\\
Proseguiamo controllando se un blocco ha stati non equivalenti, se sì li dividiamo in blocchi  distinti, separando tra di loro gli stati non equivalenti, se no, analizziamo altri blocchi. Ripetiamo fino a quando non vi sono più blocchi con stati non equivalenti

In maniera formale, se tutti gli stati \(B_{i}=\{s_{1},...,s_{k}\}\)di un blocco sono equivalenti, per ogni a transizione \(a \in A\) il target di tale transazione appartiene allo stesso blocco.
Il blocco \(B_{i}\) può essere spezzato in due se per qualche \(s,t \in B_{i}\) vale \(\textrm{move}_{d}(s,a)\in B_{j}\) e \(\textrm{move}_{d}(t,a)\notin B_{j}\)

La divisione consiste di \(B_{i}\) considerando \(a,B_{j}\) consiste nel sostituire \(B_{i}\) con due blocchi, \( B_{i1} = {s \in B_{j} | \textrm{move}_{d}(s,a)\in B}\) e \( B_{i2} = {s \in B_{j} | \textrm{move}_{d}(s,a)\notin B}\)
\begin{figure}[H]
	\centering
...
\begin{figure}[H]
	...
	\caption{Esercizio 1}
\end{figure}
Iniziamo con due blocchi iniziali, \(B_{1}=\{E\}\) dato dallo stato finale e \(B_{2}=\{A,B,C,D\}\)
Notiamo che \((b,B_{1})\) spezza \(B_{2}\), da cui abbiamo due blocchi \(B_{21}=\{D\}\) e \(B_{22}=\{A,B,C\}\)
Notiamo ancora che è possibile spezzare \(B_{22}\) in \(B_{221}=\{B\}\) e \(B_{222}=\{A,C\}\)

Da cui abbiamo il minimo 
\begin{figure}[H]
	\centering
	\includegraphics[width=.8\textwidth,keepaspectratio]{dfa_grafo_1.png}
	\label{es_1_sol}
	\caption{Esercizio 1, MIN DFA}
\end{figure}
...
\begin{figure}[H]
	...
	\caption{Esercizio 2}
\end{figure}
In questo caso, la funzione di transizione non è \emph{totale}. Quindi dobbiamo trasformarla in una funzione di transizione \emph{totale} aggiungendo un nodo \(Sink\) a cui facciamo arrivare ogni coppia stato,simbolo mancante dalla funzione \emph{parziale}
\begin{figure}[H]
	\centering
	\includegraphics[width=.8\textwidth,keepaspectratio]{dfa_grafo_3.png}
	\label{es_2_tot}
	\caption{Esercizio 2, totale}
\end{figure}
Dopo aver realizzato questo, iniziamo con
\(B_{1}=\{D\}\}\) e \(B_{2}=\{A,B,D,Sink\}\)
possiamo fare uno \emph{split} di \(B_{2}\) in quanto \(\textrm{move}_{d}(A,a) \in B_{2} \) e \(\textrm{move}_{d}(B,a) \notin B_{2} \)
da cui otteniamo i nuovi blocchi \(B_{21}=\{A,Sink\}\) e \(B_{22}=\{B,C\}\).
Possiamo fare un ulteriore \emph{split} di \(B_{22}\) in quanto \(\textrm{move}_{d}(B,b) \in B_{22} \) e \(\textrm{move}_{d}(C,b) \notin B_{22} \)
da cui otteniamo i blocchi \(B_{221}=\{B\}\) e \(B_{222}=\{C\}\).
Infine notiamo che è possibile uno \emph{split} di \(B_{21}\) in quanto \(\textrm{move}_{d}(A,a) \in B_{221} \) e \(\textrm{move}_{d}(Sink,a) \notin B_{221} \). 
Otteniamo i blocchi \(B_{211}=\{A\}\) e \(B_{212}=\{Sink\}\)
La situazione finale diventa 
\(B_{1}=\{D\}\}\), \(B_{211}=\{A\}\), \(B_{212}=\{Sink\}\), \(B_{221}=\{B\}\), \(B_{222}=\{C\}\).
Da questo eliminiamo il blocco \(B_{212}\) in quanto \emph{Sink} e possiamo costruire il nostro nuovo min-DFA, che è lo stesso di partenza. Il DFA era già minimo.  
\begin{figure}[H]
	\centering
	\includegraphics[width=.8\textwidth,keepaspectratio]{dfa_grafo_2.png}
	\label{es_2_sol}
	\caption{Esercizio 2, MIN DFA}
\end{figure}
\subsection{Esercizio 3}
\begin{figure}[H]
	\centering
	\includegraphics[width=.8\textwidth,keepaspectratio]{nfa_grafo_2.png}
	\label{es_3}
	\caption{Esercizio 3}
\end{figure}
...
\begin{figure}[H]
	...
	\label{es_3_tab}
	\caption{Esercizio 3, tabella}
\end{figure}
A questo punto dobbiamo costruirne il DFA e trovarne il minimo
\end{document}