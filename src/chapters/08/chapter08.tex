...
\begin{align*}
    ...
    % \\
    % & & E &\to E \cdot + E & E &\to E \cdot + E \\
    % & & E &\to E \cdot * E & E &\to E \cdot * E
\end{align*}
Tutto questo è fantastico, ma purtroppo c'è un grosso elefante nella nostra stanza. L'intrinseca ambiguità di Eq.\ref{eq:ex2-sh/re-grammar} ha un effetto collaterale piuttosto sgradevole: la sua tabella di parsing SLR(1) (calcolata automaticamente dallo stesso algoritmo che fa aprire esse3 a Riccardo Peron quando passa un esame) avrà dei conflitti, come possiamo ben vedere in Tab.\ref{tab:ex2-sh/re-table}.
\begin{figure}[H]
    \center
...
\end{figure}
\noindent Proviamo a capire bene qual è il punto di tutta questa situazione. Se il nostro parser si trova nello stato 5, questo vuol dire che, in testa alla nostra pila dei simboli \(symSt\), avremo \(E + E\); possiamo vederlo facilmente, è sufficiente buttare un occhio al nostro automa caratteristico (Fig.\ref{fig:ex2-sh/re-automata}) per renderci conto che lo stato 5 è raggiungibile attraverso il percorso \(0, 1, 3, 5\), o anche \(3, 5, 3, 5\). Il discorso è del tutto analogo se passiamo allo stato 6, dove avremo la testa di \(symSt\) popolata di \(E * E\), poiché 6 è raggiungibile tramite \(0, 1, 4, 6\) (o \(4, 6, 4, 6\)).
...
\begin{itemize}
    ...
    \item \(M[6, *]\): vediamo subito questa cella perché presenta una situazione del tutto analoga a quella appena vista; questa volta l'operatore coinvolto è \(\ast\), che al pari di \(+\) è left-associativo, di conseguenza anche qui scegliamo la reduce \(r2\);
...
È bene far notare, inoltre, che questo procedimento ci ha portato ad aggiungere il non-terminale \(T\) alla grammatica. Certo, un non-terminale in più non ha tutto questo grande impatto sull'efficienza degli algoritmi, ma in altre situazioni la scrittura di una grammatica che generi un linguaggio equivalente può rivelarsi davvero complicata e costosa; per questo motivo, a volte si preferisce semplicemente tenere le grammatiche ambigue e gestire i conflitti tramite direttive per il parser.
