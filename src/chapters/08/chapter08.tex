...
\paragraph{Inizio}
In primis, definiamo lo stato iniziale come \(P_0 = closure_0 (\{S' \to \cdot S\})\), dove \(S'\) è un carattere inserito da noi, mentre \(S\) è lo start symbol della nostra grammatica.

\paragraph{Svolgimento}
Tra gli stati che abbiamo già collezionato, ne esisterà uno, \(P\), che contiene un item \(A \to \alpha \cdot x \beta\), che rappresenta uno stato in cui ho già visto \(\alpha\) e in cui posso fare una transizione a \(x\beta\).

Esiste quindi una transizione da \(P\) a uno stato \(P'\) che contiene l'item \(A \to \alpha x \cdot \beta\); se \(x\) è un terminale, tale transizione rappresenta un'operazione di shift (come abbiamo visto nell'esercizio della sezione scorsa). Questo significa che avrò una transizione etichettata con \(x\) che mi porta da \(P\) a \(P'\).
...
Nonostante la grandezza della parsing table dipenda dal tipo di algoritmo di parsing bottom-up che andiamo ad applicare (i.e. SLR(1), LALR(1), LR(1)) e dalla funzione di Look Ahead (\(\mathcal{LA}\)), l'algoritmo che viene utilizzato per riempire la tabella rimane quello visto in precedenza.

Visto che abbiamo già potuto modo di discutere di tabelle di parsing anche per il parsing di tipo top-down, sembra il minimo effettuare una comparazione chiedendosi se, anche in questo caso, sia possibile ottenere dei \textbf{conflitti} (i.e. delle entries multiple defined).

Questa situazione può verificarsi anche in questo caso in due differenti modalità: parleremo di

\begin{itemize}
    \item \textbf{s/r conflict} (o shift/reduce conflict) nel caso in cui almeno un entry della tabella di parsing contenga sia un'operazione di \texttt{shift} Q (data dal fatto che esiste una \(Y\)-transizione che va dallo stato \(P\), dove ci troviamo attualmente, allo stato \(Q\)) che un'operazione di \texttt{reduce} \(A \rightarrow \beta\) (poichè \(P\) contiene un reducing item per \(A \rightarrow \beta\) e \(Y \in \mathcal{LA}(P, A \rightarrow \beta)\))
...
La prossima domanda sorgerà spontanea: cosa accade al parsing nel caso in cui vi sia un conflitto?

Visto che nell'ambito del parsing di tipo bottom up abbiamo già parlato di tre differenti classi di grammatiche è corretto analizzare ogni caso singolarmente: se troviamo almeno un conflitto (i.e. una entry multiple defined) in una tabella di parsing costruita per la grammatica \(\G\) per eseguire un parsing di tipo 
\begin{itemize}
...
Il parsing di tipo \(SLR(1)\) è un parsing meno raffinato per il semplice fatto che le tabelle contengono meno informazioni: questo deriva dal fatto che
...
Come già detto precedentemente la grammatica \(\G\) è \(SLR(1)\) se e solo se la parsing table ottenuta come descritto non ha conflitti.
...
Tale grammatica è già stata trattata precedentemente per la costruzione dell'automa caratteristico ma, come già detto, il nostro obbiettivo stavolta è quello di costruire la tabella di parsing e, per fare ciò, utilizzeremo i risultati ottenuti in precedenza: sappiamo infatti che lo stato 1 (anche indicato dal colore verde) è quello che contiene l'\textbf{Accepting Item} mentre invece gli stati contenenti i \textbf{Reducing Item} (segnalati come stati finali) sono invece 4, 7, 8, 9. 
...
Avendo a disposizione l'automa caratteristico e i dati sopra citati è dunque possibile costruire la tabella di parsing seguendo l'algoritmo per la costruzione di una parsing table per il bottom-up parsing. Procediamo quindi nel seguente modo

\begin{enumerate}
    \item Creiamo una tabella con tante righe quanti sono gli stati presenti all'interno del nostro automa caratteristico (in questo caso da 0 a 9) e con tante colonne quanti sono i terminali (a cui va aggiunto il \$) e i non terminali presenti all'interno della nostra grammatica (in questo caso abbiamo di conseguenza \emph{a b c d e \$ A B S})
    \item Facendo riferimento all'automa caratteristico andiamo ad inserire in \(M[P, Y]\) un'operazione di \texttt{shift} \(Q\) nel caso in cui \(Y\) sia un terminale e vi sia una transizione \(\tau (P, Y) = Q\), i.e. una transizione che va dallo stato \(P\) con un arco con etichetta \(Y\) allo stato \(Q\) (e.g. nel nostro caso andremo ad inserire \(M[0, a]\) = 2)
    \item A questo punto è necessario computare i \emph{follow} (e di conseguenza i \emph{first}) in quanto per poter inserire correttamente le operazioni di \texttt{reduce} abbiamo bisogno di avere questo tipo di informazioni per la funzione di Lookahead (\(\mathcal{LA}\))
\begin{table}[H]
    \centering
...
\end{table}
    \item Una volta calcolati i follow è dunque è possibile inserire un'operazione di \texttt{reduce} \(A \rightarrow \beta\) in \(M[P, Y]\) nel caso in cui \(P\) sia un stato contenente un reducing item (quindi nel nostro caso sia uno tra 4, 7, 8 o 9) e per tutti quei terminali \(Y \in \mathcal{LA}(P, A \rightarrow \beta) = follow(A)\) (e.g. \(M[4, b] = A \rightarrow \beta \cdot\) in quanto \(A \rightarrow b \cdot\) è un reducing item per lo stato 4 e \(b \in follow(A)\))
    \item Inseriamo in posizione \(M[1, \$]\) l'operazione di \texttt{Accept} in quanto 1 contiene un accepting item
    \item A questo punto dovremmo valorizzare tutte quelle celle \(M[P, Y]\) dove \(Y\) è un terminale o \$ che non ricade nei casi precedenti con il valore \emph{error()}, tuttavia per una maggiore comprensione a livello visivo verranno lasciate vuote
    \item Infine inseriamo un'operazione del tipo \texttt{Goto} \(Q\) per tutti quelle celle \(M[P, Y]\) con \(Y\) non terminale e per cui esiste è una transizione \(\tau(P, Y) = Q\) (e.g. \(M[0, S]\) = \texttt{Goto} 1 come è anche osservabile dall'automa caratteristico)
\end{enumerate}

Prima di presentare il risultato finale ci teniamo a sottolineare alcune convenzioni che verranno impiegate per motivi di, come ad esempio:
\begin{itemize}
    \item L'operazione di \texttt{shift} \(Q\) verrà indicata con \(sQ\)
    \item quella di \texttt{reduce} \(A \rightarrow \beta\) con \(rK\) dove \(K\) indica il \(K\)-esimo reducing item
    \item Acc indica l'accepting item
    \item ed infine l'operazione di \texttt{Goto} \(Q\) verrà indicata semplicemente da Q
\end{itemize}
...
La grammatica che abbiamo visto è SLR(1) e lo concludiamo perchè la grammatica da origine ad una grammatica senza conflitti.

Prima di passare alla prossima sezione è necessario precisare che \emph{solamente} le celle vuote che hanno per colonna un terminale o il dollaro sarebbero valorizzate ad \emph{error()}: infatti le celle vuote che hanno per colonna un non terminale rimangono semplicemente vuote in quanto sono dei casi che non è possibile che si verifichino. 

\subsection{Algoritmo Shift/Reduce}

\begin{itemize}
    \item \textbf{Input}: Una stringa \(w\) e la tabella di parsing \(M\) per \(\G = (V, T, S, \P)\)
    \item \text{Output}: Una derivazione di \(w\) in ordine inverso (i.e. dobbiamo leggere l'output che produciamo al rovescio: l'ultima reduce verrà utilizzata come prima) se \(w \in \mathcal{L}(\G)\), \emph{error()} altrimenti
    \item \textbf{Inizializzazione}: Per l'algoritmo è infine necessario inserire all'interno dell'input buffer la stringa \(w\$\) ed inoltre utilizzare due strutture dati:
    \begin{itemize}
        \item \(stSt\) (state stack - pila degli stati), che viene inizializzata ponendo in cima lo stato \(P_0\) (i.e. prima chiusura dell'insieme di LR(0)-item)
