...
Quando andiamo a calcolare la chiusura di un item di tipo LR(1) utilizziamo la \(closure_1(item)\) e non la \(closure_0(item)\), che funziona esattamente come la \(closure_0\) ma in più ci permette di aggiornare l'insieme \(\Delta\), il lookahead set. Questo insieme ci torna poi utile quando dobbiamo inserire mosse di riduzione in qualche stato dell'atuoma caratteristico: il lookahead set specifica quali caratteri ci dobbiamo aspettare di vedere in lettura quando stiamo per utilizzare una mossa di reduce.

Ma specifichiamo ora meglio come è strutturata l'operazione di \(closure_1(item)\):
\begin{itemize}
    \item la prima parte della chiusura è in tutto e per tutto uguale alla \(closure_0(item)\), quindi quando si calcola una \(closure_1(item)\) la prima cosa che si fa è appunto calcolare la \(closure_0(item)\), utilizzando la larte LR(0) dell'item LR(1) che stiamo chiudendo;
...
\begin{definition}
    ...
    \begin{align*}
        ...
        &[A \rightarrow \alpha \cdot B \beta, \Delta] \in closure_1(P)\\ 
        &\textrm{ and } B \rightarrow \gamma \in \P' \\
        &\textrm{ and } first(\beta \Delta) \subseteq \Gamma\}
    \end{align*}  
...
\begin{figure}[H]
	\centering
    \includegraphics[width=.9\textwidth]{lalr-automata.png}
