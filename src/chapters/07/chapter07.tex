...
\section{Il parsing}
Il parsing (o analisi sintattica) è quel processo in cui, data una grammatica \(\G = (V, T, S, \mathcal{P})\) e una parola \(w\), dobbiamo dire se \(w \in \mathcal{L(G)}\) e, se così fosse, fornire il suo albero di derivazione. Solitamente gli approcci al parsing che vengono presi in considerazione nell'ambito dei linguaggi di programmazione sono due: 
\begin{itemize}
...
Ricordiamoci che non è possibile che più produzioni possano essere inserite all'interno della stessa cella, in quanto ci stiamo occupando delle grammatiche \(LL(1)\), per cui è possibile applicare un parsing di tipo deterministico. 
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \item però, dal momento che \(\varepsilon \in first(\beta)\), entro anche nel secondo \texttt{if} e aggiungo i \(follow(B)\) ai \(follow(C)\); visto che non posso computare questa operazione fino a quando non avrò esaminato tutte le altre produzioni, lascio un placeholder nella tabella. Avendo raggiunto la fine del ciclo \texttt{while} chiudo qui il branch;
...
Prima di tuffarci nei calcoli, è interessante osservare più da vicino qual è l’idea sottesa all'inserimento di \(\$\) in follow(\(S\)) come primo passo.
...
Ricordiamo brevemente che le tabelle di parsing ci servono per verificare se, data una certa parola, questa può essere derivata tramite derivazione canonica da una data grammatica.
...
Si osservi che \(follow(A)\) può contenere il simbolo \(\$\), ed è questo il motivo per cui nel punto due usiamo \(x\) invece che \(b\); infatti, mentre \(b\) indica terminali, \(x\) serve proprio a far notare che potrebbe esserci \(\$\), che non è un terminale della grammatica, ma il carattere segnalatore della terminazione di una parola.
...
\begin{itemize}
    ...
    \item ripetiamo che è possibile finire con l'avere con due entry in una stessa cella; tuttavia, le grammatiche che compongono tabelle con questa forma non sono deterministiche e non appartengono alla classe \(LL(1)\), di cui ci stiamo attualmente occupando;
    \item le caselle con entry multiple nella tabella di parsing si dicono \emph{entry multiply-defined}.
\end{itemize}
...
Torniamo a noi: la risposta alla domanda posta in precedenza è che la grammatica in \ref{non-ll1_grammar} non appartiene alla classe \(LL(1)\); ciò si può dimostrare provando a creare la parsing table per tale grammatica.
