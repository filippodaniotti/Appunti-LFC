...
Di fatto quello che succede all'interno del \texttt{foreach} centrale è che se l'item è nuovo vado ad aggiungerlo con il suo \(\Delta\), se l'item è già presente vado vado ad aggiornare il suo lookahead set \(\Delta\). Il procedimento potrebbe risultare un po' ostico, ma sarà tutto più chiaro dopo qualche esercizio esplicativo, come al solito.
...
\begin{enumerate}
    ...
    \item[...]
    \item[6.] Ovviamente la parte interessante dell'esercizio è verificare se siamo riusciti a risolvere il conflitto che occorreva nel parsing SLR. In modo pressoché analogo a quanto accadeva, abbiamo che il kernel per lo stato 6 è :
    \begin{align*}
        [A &\to c \cdot, \{d\}] \\
        [B &\to c \cdot, \{e\}]
    \end{align*}
    Anche questa volta nello stato sono presenti due item di riduzione, ma non troviamo più il conflitto che avevamo con il parsing LR(0), perché in questo caso la riduzione \(A \to c\) si applica solo se nell'input buffer si sta leggendo \(d\), mentre la riduzione \(B \to c\) si applica solo nel caso in cui nell'input buffer si sta leggendo \(e\). Questo implica che nello stato 6, grazie al parsing LR(1), è stata eliminata l'ambiguità del conflitto r/r.
\end{enumerate}
...
\begin{align*}
    ...
    [L &\to \cdot id, \{\$\}]
\end{align*}
Visto che però avevamo già inserito due LR(1)-item con lo stesso LR(0)-item (la parte \(L \to \cdot qualcosa\)) andiamo semplicemente ad aggiornare i lookahead-set corrispondenti. Il risultato finale per la chiusura degli item dello stato iniziale è:
\begin{align*}
    [&S'\to \cdot S, \{\$\}] \\
    [&S \to \cdot L = R, \{\$\}] \\
    [&S \to \cdot R, \{\$\}] \\
...
\begin{figure}[H]
    ...
    \caption{Automa di tipo LR(0), si noti il conflitto r/r sullo stato 6}
...
È però notevole il fatto che il numero di stati sia aumentato parecchio: questo è quello che succede quando si va ad utilizzare tecniche di parsing più precise e raffinate, Larry.
...
%TODO: SCRIVERE IL PROCEDIMENTO

\paragraph{Definizione stati e costruzione dell'automa}
Il primo passo, naturalmente, è la costruzione dell'automa caratteristico. Per fare questo è necessario prima definire come sono fatti gli stati del nostro automa, ossia quali LR(1)-items comprenderanno; è giunto il momento di sfoderare la nostra fida \(closure_1\).
\begin{figure}[H]
    \centering
    \includegraphics[width=.8\textwidth]{send_help_1.png}
    \caption{help}
\end{figure}
% Descrizione degli stati? Lo stato 7: Il kernel dello stato 7 non presenta possibilità di chiusura, ma conterrà riduzione, passiamo oltre.
\begin{figure}[H]
    \centering
...
\begin{align*}
    ...
    symSt &= L 
\end{align*}
Questo è lo stato in cui abbiamo l'indeterminaione: dobbiamo scegliere tra shift o reduce. Che fare? Non possiamo più affidarci a ragionamenti pigri come quello che abbiamo fatto prima, ma magari ci va bene uguale; scegliamo di fare uno shift e andiamo avanti impavidi, diretti verso la gloria o verso il baratro. Arriviamo quindi a 6.
\begin{align*}
    w &= \underline{id\!=}id\$ \\
    stSt &= 0\;2\;6 \\
    symSt &= L\!=
...
\paragraph*{}
Il parsing di tipo SLR(1) non riconosce dunque la differenza fra lo stato 2 e lo stato 8 a differenza del parsing LR(1). Quindi, il problema del parsing SLR(1) (in questo caso) è che andiamo a piazzare un'operazione di riduzione per tutti i follow dei driver mentre in quello di tipo LR(1) lo si fa per sottoinsiemi dei follow dei driver aumentando quindi la precisione della procedura ma pagando il prezzo di una costruzione più costosa.
...
\subparagraph*{}
Per mantenere la precisione e l'affidabilità del parsing LR(1) ma senza rinunciare all'efficienza di quello SLR(1) possiamo utilizzare una terza via, il parsing LALR(1): ques'ultimo utilizza gli stati in maniera simile al parsing SLR(1), ma utilizza una funzione di lookahead un filo più raffinata. Andiamo a conoscerla più da vicino

\subsection{L'Automa caratteristico LRm(1)}

L'automa LRm(1) \(\mathcal{AM}\) è costruito a partire dall'automa LR(1) \(\mathcal{A}\) (la lettera \(\mathcal{M}\) sta per \emph{merged}).
...
Il vantaggio della tabella di parsing LALR(1) è di essere più potente (i.e. espressiva) della tabella di parsing SLR(1) pur rimanendo con delle dimensioni contenute rispetto alla LR(1). 
...
Il nostro obbiettivo è quello di costruire la parsing table LALR(1) per la grammatica di cui sopra: per poter procedere dobbiamo però prima costruire l'automa caratteristico.
...
Abbiamo visto come la modalità più semplice per ottenere le tabelle di parsing LALR sia quella di creare in primis un'automa LR(1) e poi tradurlo in LRm(1).

Ora vedremo un altro metodo che prevede di eliminare il passaggio dall'automa LR(1) e di creare fin da subito un automa con la stessa struttura dell'automa LRm(1).
Quello che andremo a creare è detto automa simbolico, preché utilizza dei simboli (delle variabili) al posto dei lookahead set; una volta terminata la costruzione dell'automa simbolico risolvendo i simboli si va a calcolare quelli che poi saranno i lookahead set dell'automa LRm(1).
...
\begin{figure}[H]
    ...
    \caption{Stato 0: sulla sinistra automa simbolico, sulla destra LR(1)}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \begin{align*}
            ...
            L &\to \cdot id, \; \{=\}
        \end{align*}
        in questo caso, dato che \(\beta \neq \varepsilon\), vediamo come il lookahead set corrisponda ad un carattere e non una variabile, questo è possibile mentre stiamo calcolando la chiusura di item simbolici: la variabile verrà introdotta al posto del carattere quando useremo questi item come kernel per un nuovo stato;
    \end{itemize}
    ...
    \end{align*}
\end{itemize}
Il consiglio per chi non ha capito questi passaggi è quello di fare qualche esercizio di costruzione di automi LR(1) e poi riprendere da qui, fidatevi ne vale la pena!
...
\begin{figure}[H]
    ...
    \caption{Kernel stato 1: sulla sinistra automa simbolico, sulla destra LR(1)}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Kernel stato 2: sulla sinistra automa simbolico, sulla destra LR(1)}
\end{figure}
...
\begin{align*}
    Sistema:& \\
            ...
            & x_3 = x_0 \\
\end{align*}
grazie al cielo non si presentano chiusure, le transizioni invece le analizzeremo in seguito.
...
\begin{figure}[H]
    ...
    \caption{Kernel stato 3: sulla sinistra automa simbolico, sulla destra LR(1)}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Kernel stato 4: sulla sinistra automa simbolico, sulla destra LR(1)}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Kernel stato 5: sulla sinistra automa simbolico, sulla destra LR(1)}
\end{figure}
anche in questo caso  \(\beta = \varepsilon\) quindi il lookahead set del kernel è uguale a \(\Delta\), aggiorno quindi il sistema di equazioni.
\begin{align*}
    Sistema:& \\
...
\begin{figure}[H]
    ...
    \caption{Kernel stato 6: sulla sinistra automa simbolico, sulla destra LR(1)}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Kernel stato 7: sulla sinistra automa simbolico, sulla destra LR(1)}
\end{figure}
in questo caso  \(\beta = \varepsilon\) quindi il lookahead set del kernel è uguale a \(\Delta\), aggiorno quindi il sistema di equazioni.
\begin{align*}
    Sistema:& \\
...
\begin{figure}[H]
    ...
    \caption{Kernel stato 8: sulla sinistra automa simbolico, sulla destra LR(1)}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Kernel stato 9: sulla sinistra automa simbolico, sulla destra LR(1)}
\end{figure}
...
Oh butei, ghe l'avem fatta. Abbiamo finito di analizzare l'automa, quindi andiamo a risolvere lo strascico di equazioni che ci siamo tirati dietro per tutto l'esercizio:
\begin{align*}
...
\begin{align*}
    ...
    S &\to a \cdot b, x_7
\end{align*}
Ma c'è un colpo di scena, perché noi abbiamo già visto questo kernel LR(0) una volta: è esattamente il kernel dello stato 2!
