...
\begin{document}
    ...
    \draw[] (A1.south) node[isosceles triangle, rotate=90, isosceles triangle apex angle=100, draw, minimum size=0.71cm, anchor=apex, fill=green!70] (T1) {};
    
    \draw[] ([yshift=-0.2cm,xshift=0.3cm]T.left corner) node[](u){\textbf{u}};
    \draw[] ([yshift=-0.2cm]T2.118) node[color=green!70](v){\textbf{v}};
    \draw[] ([yshift=-0.2cm]T1.lower side) node[color=green!70](w){\textbf{w}};
