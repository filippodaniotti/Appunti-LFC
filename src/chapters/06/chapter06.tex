...
I file con estensione \texttt{.l} che diamo in pasto a Flex hanno la seguente struttura:
\begin{minted}[breaklines, tabsize = 2]{bash}
...
//(Preambolo)
% { code
...
\end{itemize}
In sostanza, questo file farà sì che, nella lettura, qualsiasi carattere incontriamo, ad eccezione del new line, verrà sostituito da un'occorrenza della stringa \texttt{"hello world!"}; quindi, ad esempio:
\begin{itemize}
...
\begin{enumerate}
    \item il primo blocco (righe 1 e 2, in questo caso vuoto) è identificato dal simbolo di apertura \texttt{\%\{} a cui può seguire del codice C;
