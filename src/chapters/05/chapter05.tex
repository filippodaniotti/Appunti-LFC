...
Qui inizia il corpo centrale dell'algoritmo: questo ciclo serve per iterare su tutti gli stati che si stanno inserendo mano a mano nell'insieme \(R\) degli stati del DFA. 
...
La divisione consiste di \(B_{i}\) considerando (\(a,B_{j}\)) consiste nel sostituire \(B_{i}\) con due blocchi, \( B_{i1} = \{s \in B_{i} \mid move_{d}(s,a)\in B\}\) e \( B_{i2} = \{s \in B_{i} \mid move_{d}(s,a)\notin B\}\)
...
\paragraph{Unione}
Se prendiamo due linguaggi regolari e ne operiamo l'unione, il risultato di questa è ancora un linguaggio regolare. Come nel caso precedente, essendo che i linguaggi sono regolari, allora è possibile costruire un DFA sia per il primo che per il secondo; se a questo punto si prendono i due automi così costruiti e li si lega tramite una \(\varepsilon\)-transizione, per l'operazione di \emph{alternanza} si ottiene nuovamente 
